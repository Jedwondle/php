...
function emailExists($emailaddress){
 ...
 $sql = 'SELECT auth_id FROM authentication WHERE auth_email = ?';
 if($stmt = $conn->prepare($sql)){
  $stmt->bind_param('s',$emailaddress);
  $stmt->bind_result($authid);
  $stmt->execute();
  $stmt->fetch();
  $stmt->close();
 }
...
function loginUser($email, $password){
 ...
 $sql = 'SELECT client_id, client_first, client_last, auth_email, auth_rights FROM client INNER JOIN authentication ON client.client_id = authentication.client_id WHERE auth_email = ? AND auth_password = ?';
